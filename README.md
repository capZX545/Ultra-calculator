...
Then open port 5000. The APK is `UltraCalculator.apk` at the repo root (same file under `phone/`). Package `org.capzx545.ultracalculator`, minSdk 24, versionName 1.8.4, versionCode 13. Uninstall an older build first if the installer complains. Every GitHub release attaches the Windows installer and the Android APK together.
...
- **Android:** install `UltraCalculator-1.8.4.apk`. Uninstall an older build first if the phone refuses it.
