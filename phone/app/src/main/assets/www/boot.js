(async function () {
  ...
  async function startEngine() {
    if (window._engineStarted) return;
    ...
    try {
      ...
      if (window.PACKED) {
        pyodide.FS.writeFile("formulas.json", JSON.stringify({
          categories: window.PACKED.categories,
          formulas: window.PACKED.formulas
        }));
      } else {
  ...
  if (typeof requestIdleCallback === "function") {
    requestIdleCallback(function () { setTimeout(startEngine, 700); }, { timeout: 2500 });
  } else {
    setTimeout(startEngine, 900);
  }
})();
